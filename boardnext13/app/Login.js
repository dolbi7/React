...
const Login = () =>{
    ...
    return(
        <div>
            <input type="text" ref={emailRef} value={email} className={styles.email} 
                    onChange={(e) => setEmail(e.target.value)} placeholder="이메일을 입력하세요." />
            <input type="password" ref={passwordRef} value={password} className={styles.memberpasswd}  
            ...
            <br />
            <input type="button" className={styles.login_btn} value="로그인" onClick={loginCheck} />  
        </div>
    )
}
